...
export function splitByParagraphs(sourceText: string): string[] {
...
export function splitBySceneMarkers(sourceText: string): string[] {
...
export function hasSceneMarkers(sourceText: string): boolean {
  return normalizeLineEndings(sourceText)
    .split('\n')
    .some((line) => sceneBreakLine.test(line));
}

export function countSentences(sourceText: string): number {
...
export function findMemoryExample(sourceText: string) {
  return knownTranslationMemory.get(sourceText);
}

export function replaceExactPhrase(sourceText: string, phraseMap: Map<string, string>): string {
...
export function buildFallbackFaithfulDraft(sourceText: string): string {
  return replaceExactPhrase(sourceText, faithfulPhraseMap);
}

export function buildFallbackVoiceDraft(faithfulDraft: string): string {
  return replaceExactPhrase(faithfulDraft, voicePhraseMap);
}

export function buildFallbackPolishedDraft(voiceDraft: string): string {
  return replaceExactPhrase(voiceDraft, polishedPhraseMap);
}

export function buildFallbackLiteraryNaturalnessDraft(voiceDraft: string): string {
  return replaceExactPhrase(voiceDraft, naturalnessPhraseMap);
}

export function buildFallbackProfessionalLiteraryCopyeditDraft(
  sourceText: string,
